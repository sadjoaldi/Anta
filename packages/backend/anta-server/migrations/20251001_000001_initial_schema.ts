...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable("drivers", (table) => {
    ...
    table
      .enu("status", ["offline", "online", "busy", "suspended"])
      .notNullable()
      .defaultTo("offline");
    table
      .enu("kyc_status", ["pending", "approved", "rejected"])
      .notNullable()
      .defaultTo("pending");
  ...
  await knex.schema.createTable("vehicles", (table) => {
    ...
    table
      .enu("status", ["pending", "active", "inactive", "banned"])
      .notNullable()
      .defaultTo("active");
    table.timestamp("created_at").notNullable().defaultTo(knex.fn.now());
    table.timestamp("updated_at").notNullable().defaultTo(knex.fn.now());
  ...
  await knex.schema.createTable("payouts", (table) => {
    ...
    table.integer("amount").unsigned().notNullable();
    table
      .enu("status", ["pending", "processing", "paid", "failed"])
      .notNullable()
      .defaultTo("pending");
    table.timestamp("created_at").notNullable().defaultTo(knex.fn.now());
